...
Include ${options.maxReqsPerAgent} requirements, risks, time estimates. Be thorough.`;
  ...
  const handleStart = async () => {
    ...
    const anyChart = options.includeBarChart || options.includeRiskMatrix || options.includeBudgetPie;
    if (!anySelected && !anyChart) { setError('Select at least one section or chart.'); setStatus('error'); return; }
  ...
  const generateAndDownload = async (msgs: typeof messages) => {
    if (options.outputFormat === 'pdf') {
      ...
      const agentOutputs = contributing.map((a) => ({
        name: a.name,
        role: a.division,
        output: msgs.filter((m) => m.agentName === a.name).map((m) => m.content).join('\n'),
      }));
      
      let reportText = `# Software Requirements Document\n\n## Project: ${projectName}\n\n`;
      
      if (options.includeFunctional) reportText += msgs.filter((m) => m.agentName && m.content.includes('RF-')).map((m) => m.content).join('\n\n');
      if (options.includeNonFunctional) reportText += '\n\n## Non-Functional Requirements\nGenerated by agent analysis.\n';
      if (options.includeRisks) reportText += '\n\n## Risk Analysis\nSee risk matrix chart below.\n';
      if (options.includeTimeEstimate) reportText += '\n\n## Time Estimates\n' + agentOutputs.map((a) => `- ${a.name} (${a.role}): ${a.output.length} chars`).join('\n');
      if (options.includeConclusion) reportText += '\n\n## Conclusion\nRequirements document generated via AI-driven analysis.\n';
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          {status === 'config' && (
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 text-sm text-gray-400"><SlidersHorizontal size={16} className="text-purple-400" /> Sections & Charts</div>
              ...
              <div className="border-t border-[#1F2937] pt-2 space-y-1">
                <p className="text-[10px] text-gray-500 mb-1">Charts</p>
                <div className="flex gap-3">
                  <button onClick={() => setOptions((p) => ({ ...p, includeBarChart: !p.includeBarChart }))}
                    className={`flex items-center gap-1.5 p-2 rounded-lg border text-xs transition ${options.includeBarChart ? 'bg-purple-500/10 border-purple-500/30 text-purple-300' : 'bg-[#1A1F2E] border-[#2D3548] text-gray-500'}`}>
                    {options.includeBarChart ? <CheckSquare size={12} /> : <Square size={12} />} 📊 Costs
                  </button>
                  <button onClick={() => setOptions((p) => ({ ...p, includeRiskMatrix: !p.includeRiskMatrix }))}
                    className={`flex items-center gap-1.5 p-2 rounded-lg border text-xs transition ${options.includeRiskMatrix ? 'bg-purple-500/10 border-purple-500/30 text-purple-300' : 'bg-[#1A1F2E] border-[#2D3548] text-gray-500'}`}>
                    {options.includeRiskMatrix ? <CheckSquare size={12} /> : <Square size={12} />} ⚠️ Risks
                  </button>
                  <button onClick={() => setOptions((p) => ({ ...p, includeBudgetPie: !p.includeBudgetPie }))}
                    className={`flex items-center gap-1.5 p-2 rounded-lg border text-xs transition ${options.includeBudgetPie ? 'bg-purple-500/10 border-purple-500/30 text-purple-300' : 'bg-[#1A1F2E] border-[#2D3548] text-gray-500'}`}>
                    {options.includeBudgetPie ? <CheckSquare size={12} /> : <Square size={12} />} 🥧 Budget
                  </button>
                </div>
              </div>
              ...
              <div className="border-t border-[#1F2937] pt-2">
                <div className="flex gap-2">
                  <button onClick={() => setOptions((p) => ({ ...p, outputFormat: 'docx' }))}
                    className={`flex-1 p-2 rounded-lg border text-sm font-medium transition ${options.outputFormat === 'docx' ? 'bg-purple-500/10 border-purple-500/30 text-white' : 'bg-[#1A1F2E] border-[#2D3548] text-gray-500'}`}>
                    📄 .docx
                  </button>
                  <button onClick={() => setOptions((p) => ({ ...p, outputFormat: 'pdf' }))}
                    className={`flex-1 p-2 rounded-lg border text-sm font-medium transition ${options.outputFormat === 'pdf' ? 'bg-purple-500/10 border-purple-500/30 text-white' : 'bg-[#1A1F2E] border-[#2D3548] text-gray-500'}`}>
                    📑 .pdf
                  </button>
                </div>
              </div>
